...
def cy_hough_circle_trans(im_in, int radius_in, int d, region=None, x=None, y=None, z=None, thresh=0, int thickness=1, update=True):
    '''
    ...
    if (z is None) and (x is not None): z = np.ones_like(x)
    ...
    cdef np.ndarray[np.int16_t, ndim=1] x_in
    cdef np.ndarray[np.int16_t, ndim=1] y_in
    cdef np.ndarray[np.int32_t, ndim=1] z_in
    ...
    if region is None:
        ...
        # list of hits
            ...
            x_in, y_in, z_in = (x/d).astype(np.int16), (y/d).astype(np.int16), (z/z.max()*10**4).astype(np.int32)
            for i in range(x.size):
                xi, yi, zi = x_in[i], y_in[i], z_in[i]
                if (np.mod(i+1,100) == 0) and update: print '%d / %d' %(i+1, x.size)
                
                hough += zi * circle[(sz - xi):(csz - xi), (sz - yi):(csz - yi)].astype(np.int32)
    ...
    # only transform over given region #################################################
        ...
        # list of hits
            ...
            for i in range(x.size):
                ...
                hough[xhlc:xhrc, yhlc:yhrc] += zi * circle[xclh:xcrh, yclh:ycrh].astype(np.int32)
